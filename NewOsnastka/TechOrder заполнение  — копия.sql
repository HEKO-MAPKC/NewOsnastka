...
--insert into Osnastka.TechOrder (IsApplicationFrom,Draft, FactoryOrder, FactoryNumberOrder,WorkshopID,NameDraftProduct,TechOrder,TypeOsnast,ReasonProduction,DateLimitation,AuthorTechnolog,DateCreateApplication,
--								AuthorConstructor, IsReturnedToTechnolog, DateReturnedToTechnolog, ReasonReturnedToTechnolog, IsAtConstructor, DateAtConstructor, DateAtApproval, RepairOrProduction, DateChangeTechnolog, YearTechOrder
--								,OperationCode
--								,Workplace
--								--,Ksi						
...
--      ,cast(p.[tzak] as smallint) as TechOrder
...
--      ,LTRIM(RTRIM(cast(iif(p.[zak_1] is null, '', p.zak_1) as varchar(7)))) as YearTechOrder
...
--	  --,cast(pr.id as int)		as							OldZayvkaID		
...
--  left join ReferenceInformation r on r.ReferenceInformationID = p.nvid+2 and r.ReferenceInformationID<=10 --тут подправить
--  left join ReferenceInformation f on f.ReferenceName = rtrim(p.rem_izg) and f.ReferenceInformationID<=6 --тут подправить
...
--  where pr.zak_1 is  null
  --SELECT distinct 0 as													IsApplicationFrom
  --    ,cast(min(p.[draft]) as numeric (13,2)) as											Draft      
  --   ,cast(min(p.[zakaz]) as smallint) as							FactoryOrder
  --    ,cast(min([nom]) as smallint) as							FactoryNumberOrder
  --    ,iif (min(w.WorkshopID) is null, 1,min(w.WorkshopID)) as WorkshopID--w.WorkshopID as WorkshopID --------------------------------------------------- чтобы подтягивать цех и кладовую (cex + klad)	  
  --    ,cast(min(p.[tzak]) as smallint) as							TechOrder
  --    --,LTRIM(RTRIM(cast(p.[dop] as varchar(3500)))) as						AddInformation            
  --    ,cast(min(dza) as datetime) as								DateCreateApplication					
	 ----,cast(koltex as	smallint)		as					AmountEquipmentForOper
	 -- ,LTRIM(RTRIM(cast(min(imyt) as varchar (20))))	as						AuthorTechnolog			
	 -- ,cast(min(dizmt) as datetime)		as						DateChangeTechnolog		
	 -- ,LTRIM(RTRIM(cast(min(kpod) as varchar(5))))		as						DepartmentTechnolog		
	 -- --,LTRIM(RTRIM(cast(prim as varchar (50))))	as						Note						
	 -- ,cast(min(draftiz) as	numeric (13,2)) as					DraftProduct			
	 -- ,LTRIM(RTRIM(cast(min(ndraftiz) as varchar (50))))	as					NameDraftProduct		
	 -- ,LTRIM(RTRIM(cast(min(prim_an) as	varchar (20))))	as					Analogue				
	 -- --,LTRIM(RTRIM(cast(naosnsl as	varchar (50))))	as					NameOsnastTechnolog	
	 -- --,iif(kl = 0,null,iif(kl = 1,0,1))			as							IsHandOverDraft				
	 -- --,cast(iif(p.trud<0, 0,p.trud )as	numeric(7,2))as			LaborManufacturingAssume	
	 -- ,LTRIM(RTRIM(cast(min(imyk) as varchar(20))))	as						AuthorConstructor				
	 -- ,cast(min(dt_ko) as datetime)		as						DateWorkChiefConstructor		
  --    ,LTRIM(RTRIM(cast(iif(p.[zak_1] is null, '', p.zak_1) as varchar(7)))) as	YearTechOrder
	 -- 	,min(op.code) as OperationCode
		--,min(ob.rab_m) as Workplace

		--	  ,cast(min(tzakpred) as int)		as						OldProosID	
	 -- ,cast(min(p.id) as int)		as							OldZayvkaID	
	 -- 	  --,IIF((p.dop like '%в мет. не%' or p.dop like '%в металле не изг%'),0,1)   as IsMakeInMetal
  --FROM [FOX].[dbo].zayvka p
  --left join  Workshop w on Rtrim(LTrim(p.cexpol)) = w.Workshop 
  --left join proos pr on p.zak_1 = pr.zak_1 
...
--  select YearTechOrder,count(YearTechOrder) as c from Osnastka.TechOrder group by YearTechOrder having count(YearTechOrder)<>1 order by c desc

--  order by YearTechOrder

--select AddInformation,IsMakeInMetal from Osnastka.TechOrder
--select * from proos p 
--left join ReferenceInformation f on f.ReferenceName = rtrim(p.rem_izg)		
--where f.ReferenceName is null

--  select zak_1,trud,draft from zayvka where trud < 0

--  select * from Osnastka.TechOrder p
--    left join oborud os on os.rab_m = p.rab_m where p.rab_m is null

--	select * from zayvka where tzak >6999 order by tzak

--select DepartmentTechnolog, count(*) from Osnastka.TechOrder group by DepartmentTechnolog  order by   COUNT(*) desc                 --что делать с пустыми полями -все тримаем
--select Note							, count(*) from Osnastka.TechOrder group by Note  order by   COUNT(*) desc						--первая буква заглавная   --все тримаем
--select	NameDraftProduct			, count(*) from Osnastka.TechOrder group by NameDraftProduct  order by   COUNT(*) desc          --первая буква заглавная   --все тримаем
--select	Analogue					, count(*) from Osnastka.TechOrder group by Analogue  order by   COUNT(*) desc					--все тримаем
--select	NameOsnastTechnolog			, count(*) from Osnastka.TechOrder group by NameOsnastTechnolog  order by   COUNT(*) desc		--первая буква заглавная   --все тримаем
--select	ReasonProduction			, count(*) from Osnastka.TechOrder group by ReasonProduction  order by   COUNT(*) desc			--первая буква заглавная   --все тримаем
--select	AuthorConstructor			, count(*) from Osnastka.TechOrder group by AuthorConstructor  order by   COUNT(*) desc			--"Конструкто", Киселёв А., МНКозлов  
--select	ReasonReturnedToTechnolog, count(*) from Osnastka.TechOrder group by ReasonReturnedToTechnolog  order by   COUNT(*) desc	--первая буква заглавная   --все тримаем
--proos
...
--select zak_1,cex,klad from proos p 
--left join Workshop w on p.cex =w.Workshop where w.Workshop is null order by zak_1 desc
--select * from Osnastka.TechOrder where DateAtConstructor is not null
--select * from zayvka where zak_1 = '23-7789'
--select * from osn_act --1-99
--select TechOrderID, AddInformation,count(*) from Osnastka.DraftOsnast group by TechOrderID, AddInformation having count(*) >4 order by TechOrderID desc
--select TechOrderID, count(*) from Osnastka.DraftOsnast group by TechOrderID having count(*) >4 order by TechOrderID desc
--select * from Osnastka.DraftOsnast where TechOrderID = 10965